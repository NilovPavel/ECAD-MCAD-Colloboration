...
namespace AltiumDebugDLL
{
    public class InterFaceKind : MarshalByRefObject
    {
        private void CreateFiles(string projectPath)
        {
            ...
            using (FileStream fs = new FileStream(@"d:\TEST\BoardData\Altium\" + Path.GetFileNameWithoutExtension(projectPath) + ".xml", FileMode.Create))
            {
                formatter.Serialize(fs, board);
            }

            using (FileStream fs = new FileStream(@"d:\TEST\BoardData\Altium\" + Path.GetFileNameWithoutExtension(projectPath) + ".xml", FileMode.OpenOrCreate))
            {

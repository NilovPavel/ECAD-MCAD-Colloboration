...
namespace BoardBuilder.KompasBoardSpace.CutOut
{
    class KompasGroupCutOut
    {
        ...
        private void PaintSketch()
        {
            ...
            //Не прокатило - компас не понимает углы > 360 //this.contours.ForEach(contour => contour.Arc.ForEach(arc => sketchEdit.ksArcByAngle(arc.OriginalPoint.X, arc.OriginalPoint.Y, arc.Radius, arc.StartAngle, arc.EndAngle%360, 1, 1)));
            //Тоже не прокатило //this.contours.ForEach(contour => contour.Arc.ForEach(arc => sketchEdit.ksArcByPoint(arc.OriginalPoint.X, arc.OriginalPoint.Y, arc.Radius, arc.Point1.X, arc.Point1.Y, arc.Point2.X, arc.Point2.Y, 1, 1)));
            this.contours.ForEach(contour => contour.Arc.ForEach(arc => sketchEdit.ksArcBy3Points(arc.Point1.X, arc.Point1.Y, arc.MiddlePoint.X, arc.MiddlePoint.Y, arc.Point2.X, arc.Point2.Y, 1)));
            this.contours.ForEach(contour => contour.Arc.Where(oneArc => oneArc.SweepAngle == 360).ToList()
           .ForEach(arc => sketchEdit.ksArcByAngle(arc.OriginalPoint.X, arc.OriginalPoint.Y, arc.Radius, arc.StartAngle, arc.EndAngle % 360, 1, 1)));
